...
if __name__ == "__main__":
    ...
    parser.add_argument("--path_data_imagenet_corrupted", nargs="?",
                        help="path to corrupted ImageNet dataset")

    args = parser.parse_args()
    settings = args.settings
    folder_path_save = ifValueNotNone("./", args.folder_path_save)
    models_cms = ifValueNotNone(None,
        [str(i) for i in args.calib_models.split(",")] if args.calib_models else None)
    perturb_levels = ifValueNotNone(6, args.perturb_levels)
    ...
    experiment_settings = {
        ...
        "IMAGENET_ResNet50_OOD_GN": {
            ...
            "perturb_levels": perturb_levels, #number of pertrubation levels (epsilons)
            "test_name": "test_ds",
            "folder_path_save": ifValueNotNone("../results/Calibration_gaussian",
                                                folder_path_save),
            "path_data_imagenet": path_data_imagenet,
            "path_data_objectnet": path_data_objectnet,
            "path_data_imagenet_corrupted": path_data_imagenet_corrupted
