"""
Adapted from:
https://github.com/p-lambda/verified_calibration/blob/master/
calibration/calibrators.py (11-22-2020)
https://github.com/p-lambda/verified_calibration/blob/master/
calibration/utils.py (11-22-2020)
"""
...
class HistogramTopCalibrator:
    ...
    def train_calibration(self, probs, labels):
        ...
        probs = get_top_probs(probs)
        predictions = get_top_predictions(probs)
        correct = (predictions == labels)
        bins = get_equal_bins(probs, num_bins=self._num_bins)
        self._calibrator = get_histogram_calibrator(
            probs, correct, bins)
...
class PlattBinnerTopCalibrator:
    ...
    def train_calibration(self, probs, labels):
        ...
        probs = get_top_probs(probs)
        correct = (predictions == labels)
        self._platt = get_platt_scaler(
            probs, correct)
        platt_probs = self._platt(probs)
        bins = get_equal_bins(platt_probs, num_bins=self._num_bins)
        self._discrete_calibrator = get_discrete_calibrator(
            platt_probs, bins)

    def calibrate(self, probs):
        probs = self._platt(get_top_probs(probs))
...
class HistogramMarginalCalibrator:
    ...
    def train_calibration(self, probs, labels):
        ...
        for c in range(self._k):
            # For each class c, get the probabilities the model output for
            #that class, and whether the data point was actually class c, or not.
            probs_c = probs[:, c]
...
class PlattBinnerMarginalCalibrator:
    ...
    def train_calibration(self, probs, labels):
        ...
        for c in range(self._k):
            ...
            self._platts.append(platt_c)
            platt_probs_c = platt_c(probs_c)
            calibrator_c = HistogramBinning(bins=self._num_bins)
            calibrator_c.fit(platt_probs_c, labels_c)
            self._calibrators.append(calibrator_c)
    ...
    def calibrate(self, probs):
        ...
        for c in range(self._k):
            ...
            calibrated_probs[:, c] = self._calibrators[c].transform(platt_probs_c)
        return calibrated_probs
