...
class Imagenet:
    ...
    def load_data(self):
        ...
        self.train_dataset = self.train_dataset.map(
            lambda d: {"image": preprocess_image(d["image"], is_training=True), "label": d["label"]})
        ...
        # transform to tuple
        self.train_dataset = self.train_dataset.map(lambda d: (d["image"], d["label"]))
        self.valid_dataset = self.valid_dataset.map(lambda d: (d["image"], d["label"]))
